...
const QUOTES = [
	'🎃 Here\'s a {icon2}{item2} and a {icon}{item}\n\nWant more? Try the `farm`, `vote` commands.'
]
const OFFICIAL_QUOTES = [
	'🎃 You receieved **2x** {icon2}{item2} and **2x** {icon}{item} for playing in the official Lootcord server! 😎'
]
...
exports.command = {
	...
	async execute (app, message, { args, prefix, guildInfo, serverSideGuildId }) {
		...
		if (message.channel.guild.id === app.config.supportGuildID) {
			...
			await app.itm.addItem(message.author.id, 'small_loot_bag', 2, serverSideGuildId)
			await reply(message, OFFICIAL_QUOTES[Math.floor(Math.random() * OFFICIAL_QUOTES.length)]
				...
				.replace('{icon2}', app.itemdata.small_loot_bag.icon)
				.replace('{item2}', '`small_loot_bag`'))
		}
		else {
			await app.itm.addItem(message.author.id, 'military_crate', 1, serverSideGuildId)
			await app.itm.addItem(message.author.id, 'small_loot_bag', 1, serverSideGuildId)
