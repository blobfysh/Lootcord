...
exports.command = {
	...
	async execute (app, message, { args, prefix, guildInfo, serverSideGuildId }) {
		...
		if (hourlyCD) {
			return reply(message, `You need to wait \`${hourlyCD}\` before farming again.`)
		}
		...
		if (chance >= 96) {
			...
			await reply(message, `You decide to go ⛏️ mining and find a **RARE** ${app.itemdata.high_quality_metal.icon}\`high_quality_metal\`!`)
		}
		else {
			...
			if (rand < 0.6) {
				...
				await reply(message, `You decide to scavenge for loot and find **1x** ${app.itemdata.crate.icon}\`crate\`!`)
			}
			else if (rand < 0.7) {
				await app.itm.addItem(message.author.id, 'metal', 1, serverSideGuildId)
				await reply(message, `You decide to go ⛏️ mining and bring back **1x** ${app.itemdata.metal.icon}\`metal\`!`)
			}
			else if (rand < 0.85) {
				await app.itm.addItem(message.author.id, 'stone', 1, serverSideGuildId)
				await reply(message, `You decide to go ⛏️ mining and bring back **1x** ${app.itemdata.stone.icon}\`stone\`!`)
			}
			else {
				await app.itm.addItem(message.author.id, 'wood', 1, serverSideGuildId)
				await reply(message, `You decide to go 🪓 chop some trees and receive **1x** ${app.itemdata.wood.icon}\`wood\`!`)
			}
		}
	}
}
