...
module.exports = {
    ...
    async execute(app, message){
        ...
        if(userSpawns.length > 2 && !app.sets.adminUsers.has(message.author.id) && await app.patreonHandler.isPatron(message.author.id, 3)) return message.reply('❌ You already have spawns active!\n\nYou are limited to **3** spawn channels. If you would like to disable your active spawns, use `disablebounty`.');
