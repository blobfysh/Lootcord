...
module.exports = {
	...
	async execute (app, message, { prefix, serverSideGuildId, eventPingRole }) {
		...
		const collectorObj = app.msgCollector.createChannelCollector(message.channel.id, m => m.content.toLowerCase() === 'give me candy', { time: 40000 })
				...
				for (const user in joined) {
					if (Math.random() < 0.9) {
						...
						if (hasEnough) {
							...
							await app.itm.addItem(user, item, 1, serverSideGuildId)
