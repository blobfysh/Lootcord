module.exports = {
	...
	async execute (app, message, { prefix, serverSideGuildId, eventPingRole }) {
		...
		const collectorObj = app.msgCollector.createChannelCollector(message.channel.id, m => m.content.toLowerCase() === 'attack mummy', { time: 40000 })

		const exploreEmbed = new app.Embed()
			.setColor('#FF9A00')
			.setTitle('Event - __MUMMY ATTACK__')
			.setDescription('**A frightening mummy is on the loose!**\n\nType `attack mummy` to fight!\n\nThe more people that attack the mummy, the higher the chance of success.')
			.setImage('https://cdn.discordapp.com/attachments/497302646521069570/758495049524314133/pitchfork_mummy.png')
		...
		try {
			...
			collectorObj.collector.on('end', async reason => {
				...
				for (const user in joined) {
					...
					if (wasSuccessful) {
						...
						if (hasEnough && Math.random() < 0.07) {
							await app.itm.addItem(user, 'medium_loot_bag', 1, serverSideGuildId)

							quote = `<@${user}> received **1x** ${app.itemdata.medium_loot_bag.icon}\`medium_loot_bag\`.`
						}
						else if (hasEnough) {
							await app.itm.addItem(user, 'small_loot_bag', 1, serverSideGuildId)

							quote = `<@${user}> received **1x** ${app.itemdata.small_loot_bag.icon}\`small_loot_bag\`.`
						}
						else {
							const moneyMin = 1000
							const moneyMax = 2000
				...
				if (results.length) {
					await message.channel.createMessage(`**Event Results - __MUMMY ATTACK__**\n\n${wasSuccessful ? '✅ The attack was a success!' : '❌ The attack failed.'}\n\n${results.join('\n')}`)
				}
			})
		}
		catch (err) {
