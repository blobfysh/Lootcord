...
exports.handleCmd = function(message, prefix, lang){
    ...
    query(`SELECT * FROM scores WHERE userId = ${message.author.id}`).then(row => {
        query(`SELECT * FROM cooldowns WHERE userId = ${message.author.id}`).then(CDrow => {
            query(`SELECT * FROM userGuilds WHERE userId = ${message.author.id} && guildId = ${message.guild !== null ? message.guild.id : 0}`).then(activeRow => {
