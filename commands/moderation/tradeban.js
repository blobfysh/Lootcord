...
module.exports = {
    name: 'tradeban',
    aliases: [''],
    description: 'Bans user from trading and using clan vaults.',
    hasArgs: false,
    worksInDM: false,
    requiresAcc: false,
    guildModsOnly: false,
        ...
        if(userNameID !== undefined){
            if(banReason == ""){
                ...
                try{
                    ...
                    query("INSERT INTO tradebanned (userId, reason, date) VALUES (?, ?, ?)", [userNameID, banReason, (new Date()).getTime()]);

                    message.client.shard.broadcastEval(`this.sets.tradeBannedUsers.add('${bannedUser.id}')`);
